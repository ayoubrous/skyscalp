    ...
    export default function ChatComponent({conversationData , isVisible , isFixed}) {
        ...
        useEffect(() => {
            ...
            const fetchData = async () => {
                    try {
                        ...
                        // Fetch product information to get the ownerID
                        const response = await fetch(`http://localhost:5500/api/getProductById/${params.id}`);
                        const result = await response.json();
        ...
        }, [conversationData]); // Add params.id as a dependency
        ...
        return (
            <div className={`chat-container ${isVisible ? 'active' : ''} ${isFixed ? 'fixed-position' : ''}`} >
                <Toaster />
                <div ref={messageContainerRef} className='message-container' style={{ padding: '10px', height: '350px', overflowY: 'scroll'}}>
